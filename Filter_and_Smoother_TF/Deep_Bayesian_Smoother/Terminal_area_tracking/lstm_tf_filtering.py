...
class LSTMCell(LayerRNNCell):
  ...
  def build(self, inputs_shape):
    ...
    self._kernel_mod_l1 = self.add_variable("kernel_mod_l1",shape=[state_depth + h_depth, self._num_units],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    self._kernel_mod_l2 = self.add_variable("kernel_mod_l2",shape=[self._num_units, 4],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    ...
    self._bias_mod_l2 = self.add_variable("bias_mod_l2", shape=[4], initializer=initializer)
  ...
  def call(self, inputs, state):
    """
    Run one step of filtering of EGBRNS.
    Args:
      inputs: measurement [bt, meas_dim]
      state: A turple of (previous memory, previous state,previous covariance)
    Returns:
      - m : estimated state
      - new_state : A turple of (updated memory, estimated state, estimated covariance)
      - m_pred: filter prediction results
      - P: filter update covariance matrix
    ...
    Memory update gate
    '''
    # Memory update & measure network (The mean and variance of memory are combined into one vector)
    lstm_matrix_m = math_ops.matmul(array_ops.concat([c_prev, m_prev[:,:2]/150000,m_prev[:,2:]/1000], 1), self._kernel_mem)
    c_update = sigmoid(nn_ops.bias_add(lstm_matrix_m, self._bias_mem))
    '''
    State prediction gate
    '''
    # State prediction compensation network
    lstm_matrix_mod_l1 = math_ops.matmul(array_ops.concat([c_update, m_prev[:,:2]/150000,m_prev[:,2:]/1000], 1), self._kernel_mod_l1)
    ...
    add = reshape(nn_ops.bias_add(lstm_matrix_mod_l2, self._bias_mod_l2),[self._batch_size,4,1])
    # State prediction
    m_pred = math_ops.matmul(self._trans_model,reshape(m_prev,[self._batch_size,self._dim_state,1])) + add
    # State prediction measure network
    lstm_q_l1 = math_ops.matmul(array_ops.concat([c_update, m_prev[:,:2]/150000,m_prev[:,2:]/1000], 1), self._kernel_q_l1)
    ...
    '''
    State update gate (not use first-order network in this task)
    '''
    H_mat =self._meas_matrix(m_pred)
    v = reshape(inputs,[self._batch_size,self._dim_meas,1]) - self._meas_func(m_pred)
    # The calculation of R_est is not utilized
    lstm_r_l1 = math_ops.matmul(array_ops.concat([c_update, m_prev[:,:2]/150000,m_prev[:,2:]/1000,tf.reshape(inputs[:,0],[self._batch_size,1])/3.1415926,tf.reshape(inputs[:,1],[self._batch_size,1])/150000], 1), self._kernel_r_l1)
    ...
    S = math_ops.matmul(math_ops.matmul(H_mat,P_pred),
                        transpose(H_mat,[0,2,1])) + self._meas_noise #+R_est#
    # Calculate the filtering gain
    K = math_ops.matmul(math_ops.matmul(P_pred,transpose(H_mat,[0,2,1])),tf.linalg.inv(S))
    # State update
    ...
    new_state = (LSTMStateTuple_KF(c_update, m,P) if self._state_is_tuple else
                 array_ops.concat([c_update, m,P], 1))

    return m, new_state, m_pred, P, P_pred
