...
class LSTMCell(LayerRNNCell):
  ...
  def build(self, inputs_shape):
    ...
    self._kernel_mod_l1 = self.add_variable("kernel_mod_l1",shape=[state_depth + h_depth, self._num_units],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    self._kernel_mod_l2 = self.add_variable("kernel_mod_l2",shape=[self._num_units, 1],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    ...
    self._kernel_q_l1 = self.add_variable("kernel_q_l1",shape=[state_depth + h_depth, self._num_units],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    self._kernel_q_l2 = self.add_variable("kernel_q_l2",shape=[self._num_units, 1],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    ...
    self._kernel_r_l1 = self.add_variable("kernel_r_l1",shape=[state_depth + h_depth + meas_depth, self._num_units],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    self._kernel_r_l2 = self.add_variable("kernel_r_l2",shape=[self._num_units, 1],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    ...
    self._bias_mod_l2 = self.add_variable("bias_mod_l2", shape=[1], initializer=initializer)

    self._bias_q_l1 = self.add_variable("bias_q_l1", shape=[self._num_units], initializer=initializer)
    self._bias_q_l2 = self.add_variable("bias_q_l2", shape=[1], initializer=initializer)

    self._bias_r_l1 = self.add_variable("bias_r_l1", shape=[self._num_units], initializer=initializer)
    self._bias_r_l2 = self.add_variable("bias_r_l2", shape=[1], initializer=initializer)
  ...
  def call(self, inputs, state):
    ...
    lstm_matrix_m = math_ops.matmul(array_ops.concat([c_prev, m_prev/10], 1), self._kernel_mem)
    c_update = sigmoid(nn_ops.bias_add(lstm_matrix_m, self._bias_mem))
    '''
    State prediction gate
    '''
    # State prediction compensation network
    lstm_matrix_mod_l1 = math_ops.matmul(array_ops.concat([c_update, m_prev/10], 1), self._kernel_mod_l1)
    ...
    add = reshape(nn_ops.bias_add(lstm_matrix_mod_l2, self._bias_mod_l2),[self._batch_size,1])
    # State prediction
    m_pred = 0.5*m_prev+add
    # State prediction measure network
    lstm_q_l1 = math_ops.matmul(array_ops.concat([c_update, m_prev/10], 1), self._kernel_q_l1)
    lstm_q_l1 = sigmoid(nn_ops.bias_add(lstm_q_l1, self._bias_q_l1))
    lstm_q_l2 = math_ops.matmul(lstm_q_l1, self._kernel_q_l2)
    q_est = nn_ops.bias_add(lstm_q_l2, self._bias_q_l2)
    # Variance prediction
    P_pred = 0.5**2*P_prev+self._pro_noise+q_est
    '''
    State update gate (not use first-order network in this task)
    '''
    v = inputs-m_pred
    # Second-order compensation
    lstm_r_l1 = math_ops.matmul(array_ops.concat([c_update, m_prev/10,inputs/10],1), self._kernel_r_l1)
    lstm_r_l1 = sigmoid(nn_ops.bias_add(lstm_r_l1, self._bias_r_l1))
    lstm_r_l2 = math_ops.matmul(lstm_r_l1, self._kernel_r_l2)
    r_est = reshape(nn_ops.bias_add(lstm_r_l2, self._bias_r_l2),[self._batch_size,1])
    # Calculate the innovation covariance
    S = P_pred + self._meas_noise+r_est
    ...
    # Variance update
    P = P_pred - K * P_pred
    new_state = (LSTMStateTuple_KF(c_update, m,P) if self._state_is_tuple else array_ops.concat([c_update, m,P], 1))
