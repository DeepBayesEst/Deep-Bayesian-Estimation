...
Author: Yan Shi
Date: 2024/01
Training code of LSTM for the NCLT localization
'''
...
data_set = np.load('data_set.npy')[:36]

with tf.Session() as sess:
    tf.global_variables_initializer().run()
    for step in range(8000):
        a = np.array([i for i in range(36)])
        ...
        _,loss_print,resu = sess.run([train_step1,loss11,M_arr],feed_dict={x:meas_batch,y_:state_batch[:,:,:],c_start:c_start_,P_start:P_start_})
        if step % 100 == 0:
            print('**************************************************')
            print('Step %d'%step)
            print('Loss：',loss_print)
            print('**************************************************')
            # saver.save(sess, "./model_purelstm/model.ckpt")  #
