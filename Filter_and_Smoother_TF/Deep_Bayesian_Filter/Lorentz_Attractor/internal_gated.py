...
class LSTMCell(LayerRNNCell):
  ...
  def build(self, inputs_shape):
    ...
    self._kernel_mod_l1 = self.add_variable("kernel_mod_l1",shape=[state_depth + h_depth, self._num_units],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    self._kernel_mod_l2 = self.add_variable("kernel_mod_l2",shape=[self._num_units, 3],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    self._kernel_q_l1 = self.add_variable("kernel_q_l1",shape=[state_depth + h_depth, self._num_units],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    self._kernel_q_l2 = self.add_variable("kernel_q_l2",shape=[self._num_units, 3],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    self._kernel_r_l1 = self.add_variable("kernel_r_l1",shape=[state_depth + h_depth + meas_depth, self._num_units],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    self._kernel_r_l2 = self.add_variable("kernel_r_l2",shape=[self._num_units, 3],
        initializer=self._initializer,
        partitioner=maybe_partitioner)
    ...
    self._bias_mod_l2 = self.add_variable("bias_mod_l2", shape=[3], initializer=initializer)
    self._bias_q_l1 = self.add_variable("bias_q_l1", shape=[self._num_units], initializer=initializer)
    self._bias_q_l2 = self.add_variable("bias_q_l2", shape=[3], initializer=initializer)
    self._bias_r_l1 = self.add_variable("bias_r_l1", shape=[self._num_units], initializer=initializer)
    self._bias_r_l2 = self.add_variable("bias_r_l2", shape=[3], initializer=initializer)
  ...
  def call(self, inputs, state):
    ...
    add = reshape(nn_ops.bias_add(lstm_matrix_mod_l2, self._bias_mod_l2),[self._batch_size,3,1])
    # 执行状态预测
    F = self._trans_model(m_prev)
    m_pred = math_ops.matmul(F,reshape(m_prev,[self._batch_size,self._dim_state,1])) +add# + tf.matmul(Tao,add)
    # 过程噪声协方差辨识门
    ...
    # 执行协方差预测
    P_pred = math_ops.matmul(math_ops.matmul(F,P_prev),transpose(F,[0,2,1]))+Q_est # self._pro_noise +
    # 计算量测新息残差
    v = reshape(inputs,[self._batch_size,self._dim_meas,1]) - math_ops.matmul(self._meas_model,m_pred)
    # 量测噪声协方差辨识门
    ...
    # 计算新息协方差
    S = math_ops.matmul(math_ops.matmul(self._meas_model,P_pred),
                        transpose(self._meas_model,[0,2,1])) +self._meas_noise+ R_est## #
    ...
    #
    lstm_matrix_mup = math_ops.matmul(array_ops.concat([c_update,inputs/60,m/60], 1), self._kernel_memup)
    c = sigmoid(nn_ops.bias_add(lstm_matrix_mup, self._bias_memup))

    new_state = (LSTMStateTuple_KF(c, m,P) if self._state_is_tuple else
                 array_ops.concat([c, m,P], 1))

    return m, new_state,Q_est
