...
class LSTMCell(LayerRNNCell):
  ...
  def call(self, inputs, state):
    ...
    Memory update gate
    '''
    # Memory update & measure network (The mean and covariance of memory are combined into one vector)
    ...
    # State prediction
    m_pred = math_ops.matmul(self._trans_model,reshape(m_prev,[self._batch_size,self._dim_state,1])) + add#tf.matmul(Tao,add)
    ...
    '''
    State update gate (not use network in this task)
    '''
    # First-order compensation
    # lstm_matrix_z_l1 = math_ops.matmul(array_ops.concat([c_update, m_prev[:,:2]/150000,m_prev[:,2:]/1000,tf.reshape(inputs[:,0],[self._batch_size,1])/3.1415926,tf.reshape(inputs[:,1],[self._batch_size,1])/150000], 1), self._kernel_z_l1)
    ...
    v = reshape(inputs,[self._batch_size,self._dim_meas,1]) - (self._meas_func(m_pred))#+z_add)
    ...
    S = math_ops.matmul(math_ops.matmul(H_mat,P_pred),transpose(H_mat,[0,2,1])) + self._meas_noise#+R_est#
