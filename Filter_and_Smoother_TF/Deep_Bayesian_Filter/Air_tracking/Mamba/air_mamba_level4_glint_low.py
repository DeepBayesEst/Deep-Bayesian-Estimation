...
for i in range(all_gt.shape[0]):
    measurement[i, :, 0] = np.arctan2(meas_batch[i, :, 1], meas_batch[i, :, 0]) + (
                1 - xi) * np.random.normal(0, azi_n,timestep_size) + xi * np.random.laplace(0, azi_n * 2, timestep_size)  # 方位角
...
for epoch in range(5000):  # 训练10个epoch
    ...
    if epoch % 100 == 0:
        torch.save(model, './mamba_model_air/glint_leve_4_low/model.pth')