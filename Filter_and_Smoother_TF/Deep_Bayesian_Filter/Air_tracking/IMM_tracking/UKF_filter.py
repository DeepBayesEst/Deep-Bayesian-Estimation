...
class UnscentedKalmanFilter(object):
    ...
    def update(self, z, R=None, UT=None, hx=None, **hx_args):
        ...
        **hx_args : keyword argument
        ...
        self.zp, self.S = UT(self.sigmas_h, self.Wm, self.Wc, R, self.z_mean, self.residual_z)

        self.SI = self.inv(self.S)

        # compute cross variance of the state and the measurements
        Pxz = self.cross_variance(self.x, self.zp, self.sigmas_f, self.sigmas_h)

        self.K = dot(Pxz, self.SI)        # Kalman gain

        self.y = self.residual_z(z, self.zp)   # residual
